...
// Define the type for a row in the DataGrid
interface CourseRow {
  id: string;
  title: string;
  ratings: number;
...
const AllCourses = () => {
    ...
    { field: 'created_at', headerName: 'Created At', flex: 0.4 },
    {
      ...
      renderCell: (params: GridRenderCellParams<CourseRow>) => (
        <Link href={`/admin/edit-course/${params.row.id}`}>
          <FiEdit2 className="dark:text-white text-black mt-3.5" size={20} />
        </Link>
      ),
    },
    {
  ...
  const rows: CourseRow[] = data?.courses
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      toast.error(errorMessage.data?.message || 'An error occurred while deleting the course');
    }
